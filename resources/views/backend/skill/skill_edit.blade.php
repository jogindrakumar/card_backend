...
@section('main_content')
 ...
 {{-- Add About form here  --}}
	...
	<form action="{{route('skill.update',$skills->id)}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="controls">
        <input type="text" name="skill_name" value="{{$skills->skill_name}}" class="form-control"  value=""  > 
		@error('skill_name')
	 ...
	 <div class="form-group">
        ...
        <div class="controls">
        <input type="text" name="skill_per" value="{{$skills->skill_per}}"class="form-control"  value=""  > 
		@error('skill_per')
		<span class="text-danger">{{$message}}</span>
		@enderror
		</div>
